...
if selected == "Home":
    ...
    st.markdown('<div style="text-align: justify; fontSize: 18px; color: #696969;">This web application enables predictions of cholestasis using a methodology that combines predictive QSAR models for various hepatic transporters. These models predict in vitro concentrations, which are then extrapolated to in vivo doses using QIVIVE models and compared with maximum therapeutic doses.</div>', unsafe_allow_html=True)   
    st.write('')
    ...
    st.write('')
    st.markdown('<div style="text-align: justify; fontSize: 18px; color: #696969;">The "metamodel" function allows for predictions of cholestasis by combining information from in vivo oral dose-response data of multiple hepatic transporters (BCRP, MRP2, MRP3, MRP4, OATP1B1, OATP1B3, BSEP, P-gp), which are extrapolated from in vitro concentrations (IC50). To obtain the in vitro concentrations, individual QSAR models were developed using Morgan fingerprints and a battery of machine learning models. Subsequently, the IC50 values are extrapolated to in vivo oral doses using the HTTK tool with the calc_mc_oral_equiv function. This function employs reverse dosimetry-based in vitro-in vivo extrapolation (IVIVE) for high throughput risk screening through a PBPK model. These oral doses are then compared with the maximum doses of each compound. In this function, we can assign a small correction factor (K) to the oral doses, select the transporters, and choose the logical rule for combining the information (OR, AND, Majority). The selected metrics (Sensitivity, Specificity, MCC, Accuracy, and ROC-AUC) are shown for both training series (a set of 426 compounds collected in our published work) and test series.</div>', unsafe_allow_html=True)
        ...
        with st.spinner("Building graphs..."):
            ...
            if 'df_result' in st.session_state:
                ...
                with col2:
                    ...
                    plt.plot(df_graph3[df_graph3['dose'] == 1]['dose'], df_graph3[df_graph3['dose'] == 1]['Css'], linestyle='--') 
                    plt.plot(df_graph3['dose'], df_graph3['Css'], linestyle='--', color='black') 
                    plt.xlabel("Dose")
                    plt.ylabel("Css")
